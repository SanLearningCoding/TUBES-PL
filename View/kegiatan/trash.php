...
// AMBIL DATA KEGIATAN YANG DIHAPUS (SOFT DELETE)
...
if (!$check || $check->rowCount() === 0) {
    ...
    $query = "SELECT * FROM kegiatan_donasi WHERE is_deleted = 1 ORDER BY deleted_at DESC";
    $stmt = $db->prepare($query);
    $stmt->execute();
    $deleted_kegiatan = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
?>

<div class="detail-page-header">
    <h1><i class="fas fa-archive me-2"></i>Arsip Kegiatan</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <a href="?action=kegiatan" class="btn btn-back me-2">
            <i class="fas fa-arrow-left me-1"></i>Kembali
        </a>
        <?php if ($softDeleteAvailable && count($deleted_kegiatan) > 0): ?>
...
<div class="card">
    <div class="card-body">
        <?php if (!$softDeleteAvailable): ?>
            <div class="alert alert-warning">Fitur Arsip (soft-delete) belum diaktifkan pada tabel <code>kegiatan_donasi</code>. Jalankan migration/seed untuk menambahkan kolom <code>is_deleted</code> dan <code>deleted_at</code>.</div>
        <?php else: ?>
        <?php if ($softDeleteAvailable && count($deleted_kegiatan) > 0): ?>
        <div class="mb-3">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                    ...
                    <tr>
                        <th style="width: 30px;">
                            <input type="checkbox" id="selectAllCheckbox" class="form-check-input" 
                                   onchange="toggleSelectAll(this, 'kegiatan')">
                        </th>
                        <th>Nama Kegiatan</th>
                        <th>Tanggal</th>
                        <th>Lokasi</th>
                ...
                <tbody>
                    <?php foreach ($deleted_kegiatan as $k): ?>
                    <tr>
                        <td>
                            <input type="checkbox" class="form-check-input kegiatan-checkbox" 
                                   value="<?= $k['id_kegiatan'] ?>" 
                                   onchange="updateBulkDeleteButton('kegiatan')">
                        </td>
        ...
        <div class="text-center py-4">
            ...
            <h5 class="text-muted">Tidak ada data dalam Arsip Kegiatan</h5>
            <p class="text-muted mb-0">Data kegiatan yang diarsipkan akan muncul di sini.</p>
        </div>
        <?php endif; ?>
        <?php endif; ?>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            updateBulkDeleteButton('kegiatan');
        });
    });
    ...
    if (bulkRestoreBtn) {
        bulkRestoreBtn.addEventListener('click', function() {
            bulkRestoreSelected('kegiatan');
        });
    }
    
    if (bulkDeleteBtn) {
        bulkDeleteBtn.addEventListener('click', function() {
            bulkDeleteSelected('kegiatan');
        });
    }
});
